...
const DisplayMap: React.FC<DisplayMapProps> = ({ selectedRoute }) => {
  ...
  useEffect(() => {
    const fetchRoutes = async () => {
      try {
        const response = await fetch('https://54.253.121.220:4000/routes/index/coordinates');
        if (!response.ok) {
  ...
  const fetchBuses = async () => {
    try {
      const response = await fetch('https://54.253.121.220:4000/thingspeak/bus-location');
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
  ...
  const fetchBusPassCount = async () => {
    try {
      const response = await fetch('https://54.253.121.220:4000/thingspeak/all-bus-passengers');
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
