...
const RouteForm: React.FC = () => {
  ...
  useEffect(() => {
    const fetchRoute = async () => {
      try {
        const response = await fetch(`https://3.27.197.150:4000/routes/get-route/${id}`);
        if (!response.ok) {
  ...
  const [errors, setErrors] = useState<{ [key: string]: string }>({});
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const response = await fetch('https://3.27.197.150:4000/routes/update', {
          method: 'POST',
          headers: {
     ...
     return (
        ...
        >
          ...
          {page === 2 &&         <DrawRoute onUpdateLatLngs={setLatlngs} color={routeColor}    initialLatlngs={latlngs.length > 0 ? latlngs : undefined}/>}
